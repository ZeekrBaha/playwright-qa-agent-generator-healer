...
**Goal:** Build veriplay v1 — an OpenAI-powered autonomous QA agent that explores web apps with Playwright and emits verified test suites. Three surfaces (CLI explore, CLI heal, MCP server). All 9 weakness fixes from the qa-core code review baked in.
...
Use [qa-core's tools.ts](https://github.com/sardar-usman/qa-core-agent-openclaw/blob/main/src/agent/tools.ts) as a structural reference but: (a) get_dom returns the truncation signal shape from Step 1 test, (b) every tool function uses TS strict mode with no `as never`, (c) export types alongside.
...
Full implementation reference: copy the structure from qa-core's tools.ts but extend `summarizeDom` to compute `counts` and `truncated` per W3.
...
- [ ] **Step 2: Verify fail**, **Step 3: implement.** Use [qa-core's pom.ts](https://github.com/sardar-usman/qa-core-agent-openclaw/blob/main/src/agent/pom.ts) as reference for the POM structure but generate from `RunReport` directly. No LLM calls.
...
- [ ] **Step 2: Write `tests/unit/heal-report.test.ts`** — cover:
  ...
  - `extractUrlFromReport` uses `baseURL` when present (W7 fix vs qa-core's regex-only path)
...
- [ ] **Step 2-4:** Implement with structured tool call (`propose_selector`) instead of qa-core's `<intent>...</intent>` XML parsing. Same zod-validated pattern as Planner.
...
3. Quick start
4. Commands (`explore`, `heal`, `mcp`)
5. How it works (mermaid diagram + walkthrough)
6. **The 9 design decisions** — each with rationale and `src/file.ts:line` reference
7. veriplay vs qa-core comparison table
8. Project layout
9. Configuration
10. Attribution to qa-core-agent-openclaw
11. License + author

- [ ] **Step 1: Write `README.md`** matching the structure above. The "9 design decisions" section is the differentiator — link each W to its file.
...
- All 9 weakness fixes from the qa-core code review (W1-W9)
